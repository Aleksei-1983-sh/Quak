...
class RocketLauncher extends Weapon {
  ...
  update(dt, weaponGroup, isFiring, isReloading, reloadTimer) {
    ...
    if (isFiring) {
      ...
      weaponGroup.rotation.x = 0.2; // Очень сильное подбрасывание
      weaponGroup.position.y = 0.03;
    } else {
      weaponGroup.position.z *= 0.75;
      weaponGroup.rotation.x *= 0.75;
      weaponGroup.position.y *= 0.75;
    }
    
    if (isReloading) {
      // Медленная перезарядка - откидывание трубы
      weaponGroup.rotation.x = -0.5 * Math.sin(reloadTimer * Math.PI);
      weaponGroup.position.y = 0.1 * Math.cos(reloadTimer * Math.PI);
    }
  }
}
